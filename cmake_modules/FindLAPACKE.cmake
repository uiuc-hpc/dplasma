...
if(BLA_VENDOR STREQUAL "" OR BLA_VENDOR STREQUAL "All")
  ...
  if(CMAKE_Fortran_COMPILER_WORKS)
    ...
    check_function_exists(ztrsm_ CC_HAS_BLAS)
  endif()
  if(CC_HAS_BLAS)
...
# if(BLA_VENDOR STREQUAL "FLAME" AND BLA_STATIC)
...
if(BLAS_FOUND)
  ...
  if(CMAKE_Fortran_COMPILER_WORKS)
    ...
    check_function_exists(zgeqrf_ BLAS_HAS_LAPACK)
  endif()
  check_function_exists(cblas_zgemm BLAS_HAS_CBLAS)
  if(NOT BLAS_HAS_CBLAS)
    # BLIS requires pthreads to link statically
    set(THREADS_PREFER_PTHREAD_FLAG 1)
    find_package(Threads)
    cmake_push_check_state()
    list(APPEND CMAKE_REQUIRED_LIBRARIES Threads::Threads)
    check_function_exists(cblas_zgemm BLAS_HAS_CBLAS_PTHREADS)
    cmake_pop_check_state()
    if(BLAS_HAS_CBLAS_PTHREADS)
      list(APPEND BLAS_LIBRARIES Threads::Threads)
    endif()
  endif()
  check_function_exists(LAPACKE_zgeqrf BLAS_HAS_LAPACKE)
  cmake_pop_check_state()
endif(BLAS_FOUND)
...
foreach(_comp ${LAPACKE_FIND_COMPONENTS})
    ...
    if("${BLA_VENDOR}" STREQUAL "IBMESSL" OR NOT BLAS_HAS_LAPACK)
      ...
      if(LAPACK_FOUND)
        ...
        if(NOT "${LAPACK_LIBRARIES}" STREQUAL "")
          set(LAPACKE_LAPACK_LIB "${LAPACK_LIBRARIES}")
          list(APPEND LAPACKE_REQUIRED_VARS "LAPACKE_LAPACK_LIB")
        else()
          list(APPEND LAPACKE_REQUIRED_VARS "LAPACK_FOUND")
        endif()
      endif()
  ...
  elseif(_comp STREQUAL "CBLAS")
    if(NOT (BLAS_HAS_CBLAS OR BLAS_HAS_CBLAS_PTHREADS))
...
#  endif()
  ...
  foreach(_comp ${LAPACKE_FIND_COMPONENTS})
    if(LAPACKE_${_comp}_FOUND AND NOT TARGET LAPACKE::${_comp})
      ...
      elseif("${_comp}" STREQUAL "LAPACK")
        list(LENGTH LAPACK_LIBRARIES _len)
        if(_len GREATER 1)
          list(SUBLIST LAPACK_LIBRARIES 1 -1 _fallback_${_comp})
        else()
          set(_fallback_${_comp})
        endif()
        if(_len GREATER 0)
          list(APPEND _fallback_${_comp} "${LAPACKE_${_comp}_LIB}")
          list(GET LAPACK_LIBRARIES 0 LAPACKE_${_comp}_LIB)
        endif()
      endif()
